...
/**
  ...
  @retval     false if not implemented and true if implemented
  ...
  )
{
  ...
  if (platform_ops.vendor_ext_check) {
    return platform_ops.vendor_ext_check();
  }
  ...
  )
{
  if (platform_ops.nascent_init)
    return platform_ops.nascent_init();
 ...
 */
BOOLEAN
Edk2OpensbiPlatformColdBootAllowed (
  UINT32 HartID
  )
{
  if (platform_ops.cold_boot_allowed)
    return platform_ops.cold_boot_allowed(HartID);
  return true;
}

CONST struct sbi_platform_operations  Edk2OpensbiPlatformOps = {
  .cold_boot_allowed    = Edk2OpensbiPlatformColdBootAllowed,
