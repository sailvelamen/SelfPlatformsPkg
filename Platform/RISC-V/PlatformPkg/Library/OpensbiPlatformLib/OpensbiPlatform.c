  ...
  )
{
  ...
  fdt_for_each_subnode (cpu_offset, fdt, cpus_offset) {
    ...
    DEBUG ((DEBUG_ERROR, "%a %d generic_hart_index2id[%d] = %d, hartid = %d \n", __FUNCTION__, __LINE__, hart_count - 1, generic_hart_index2id[hart_count - 1], hartid));
  }

  platform.hart_count = hart_count;
  DEBUG ((DEBUG_ERROR, "%a %d platform.hart_count = 0x%x \n", __FUNCTION__, __LINE__, platform.hart_count));
	platform.heap_size = SBI_PLATFORM_DEFAULT_HEAP_SIZE(hart_count);
  DEBUG ((DEBUG_ERROR, "%a %d platform.heap_size  = 0x%x \n", __FUNCTION__, __LINE__, platform.heap_size));
  platform_has_mlevel_imsic = fdt_check_imsic_mlevel (fdt);
  if (platform_has_mlevel_imsic)
    DEBUG ((DEBUG_ERROR, "%a %d platform_has_mlevel_imsic = true \n", __FUNCTION__, __LINE__));
  else
    DEBUG ((DEBUG_ERROR, "%a %d platform_has_mlevel_imsic = false \n", __FUNCTION__, __LINE__));
...
struct sbi_platform  platform = {
  ...
  .platform_version  = SBI_PLATFORM_VERSION (0x1,    0x03),
  .name              = "Generic",
