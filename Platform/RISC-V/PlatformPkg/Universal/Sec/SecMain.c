  ...
  )
{
  ...
  Scratch->fw_heap_offset = 0x50000;
  Scratch->fw_heap_size   = ThisSbiPlatform->heap_size;
	if (!Scratch->fw_heap_size) {
    DEBUG ((DEBUG_ERROR, "%a %d !Scratch->fw_heap_size \n", __FUNCTION__, __LINE__));
    DEBUG ((DEBUG_ERROR, "%a %d  Scratch->fw_heap_size = 0x%lx \n", __FUNCTION__, __LINE__, Scratch->fw_heap_size));
  }
  if (Scratch->fw_heap_size & (HEAP_BASE_ALIGN - 1)) {
    DEBUG ((DEBUG_ERROR, "%a %d Scratch->fw_heap_size & (HEAP_BASE_ALIGN - 1) = TRUE \n", __FUNCTION__, __LINE__));
  }
  if (Scratch->fw_heap_offset < Scratch->fw_rw_offset) {
    DEBUG ((DEBUG_ERROR, "%a %d Scratch->fw_heap_offset < Scratch->fw_rw_offset \n", __FUNCTION__, __LINE__));
  }
  DEBUG ((DEBUG_ERROR, "%a %d Scratch->fw_size        = 0x%lx \n", __FUNCTION__, __LINE__, Scratch->fw_size));
  DEBUG ((DEBUG_ERROR, "%a %d Scratch->fw_heap_offset = 0x%lx \n", __FUNCTION__, __LINE__, Scratch->fw_heap_offset));
  DEBUG ((DEBUG_ERROR, "%a %d Scratch->fw_heap_size   = 0x%lx \n", __FUNCTION__, __LINE__, Scratch->fw_heap_size));
  DEBUG ((DEBUG_ERROR, "%a %d (Scratch->fw_heap_offset + Scratch->fw_heap_size) = 0x%lx \n", __FUNCTION__, __LINE__, (Scratch->fw_heap_offset + Scratch->fw_heap_size)));
  if (Scratch->fw_size < (Scratch->fw_heap_offset + Scratch->fw_heap_size)) {
    DEBUG ((DEBUG_ERROR, "%a %d Scratch->fw_size < (Scratch->fw_heap_offset + Scratch->fw_heap_size) \n", __FUNCTION__, __LINE__));
    DEBUG ((DEBUG_ERROR, "%a %d Scratch->fw_size = 0x%lx (Scratch->fw_heap_offset + Scratch->fw_heap_size) = 0x%lx \n", __FUNCTION__, __LINE__, (Scratch->fw_heap_offset + Scratch->fw_heap_size)));
  }
  if (Scratch->fw_heap_offset & (HEAP_BASE_ALIGN - 1)) {
    DEBUG ((DEBUG_ERROR, "%a %d Scratch->fw_heap_offset & (HEAP_BASE_ALIGN - 1) = TRUE \n", __FUNCTION__, __LINE__));
  }
DEBUG ((DEBUG_ERROR, "%a %d Scratch = 0x%LX \n", __FUNCTION__, __LINE__, Scratch));
DEBUG ((DEBUG_ERROR, "\n%a %d sbi_init() Entry! \n", __FUNCTION__, __LINE__));
  sbi_init (Scratch);
}
  ...
  )
{
  ...
  if (HartId == FixedPcdGet32 (PcdBootHartId)) {
    ...
    DEBUG ((DEBUG_ERROR, "%a %d HartId = %d \n", __FUNCTION__, __LINE__, HartId));
