...
static int starfive_jh7110_inst_init(void *fdt)
{
	...
	if (pmic_inst.adapter) {
		...
		if (!sbi_strncmp(name, "i2c", 3)) {
			...
			//
			// Temporary solution for adapting DTS
			//
			jh7110_inst.i2c_index -= 0xb;
		}
		else
			rc = SBI_EINVAL;
	}
...
static u64 starfive_jh7110_tlbr_flush_limit(const struct fdt_match *match)
{
  /*
...
static int starfive_jh7110_fdt_fixup(void *fdt, const struct fdt_match *match)
{
  /*
...
const struct platform_override starfive_jh7110 = {
  ...
  .tlbr_flush_limit = starfive_jh7110_tlbr_flush_limit,
  .cold_boot_allowed = starfive_jh7110_cold_boot_allowed,
  .fdt_fixup = starfive_jh7110_fdt_fixup,
